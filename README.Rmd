...
Let's run a bootstrap. We'll set `num_cores = 1` because the model
is small - parallel overhead is too expensive here.

```{r}
library(glmmboot)
boot_results <- BootGlmm(base_model = base_run, resamples = 999, base_data = sample_frame, num_cores = 1)
```
