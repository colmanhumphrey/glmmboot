...
test_that('BootCI returns matrix when given list from BootGlmm', {
    ...
    list_out = BootGlmm(base_model = simple_model, resamples = 20,
...
test_that('CombineResampledLists gives same result for list of lists, or one by one', {
    ...
    list_out1 = BootGlmm(base_model = simple_model, resamples = 20,
                        return_coefs_instead = TRUE)
    list_out2 = BootGlmm(base_model = simple_model, resamples = 20,
                        return_coefs_instead = TRUE)
    list_out3 = BootGlmm(base_model = simple_model, resamples = 20,
                         return_coefs_instead = TRUE)
